...
from .exceptions import CVZoneUnavailableError, OpenCVUnavailableError, PySimVerseUnavailableError
...
def run_live_camera(
    ...
    *,
    ...
    drone_controller = (
        _load_drone_gesture_controller(
            required_stable_detections=required_stable_detections,
            command_cooldown_detections=command_cooldown_detections,
            lost_tracking_land_detections=lost_tracking_land_detections,
            start_armed=start_armed,
        )
        if enable_drone_control
        else None
    )
        ...
        while True:
            ...
            if not ok:
                if drone_controller is not None:
                    drone_controller.emergency_land()
            ...
            if not hands and drone_controller is not None:
                drone_controller.handle_tracking_lost()
            for hand in hands:
            ...
            if drone_controller is not None:
                primitives = overlay.build_drone_status_overlay(drone_controller.status_text)
                _draw_overlay(cv2, cvzone, frame, primitives)

            cv2.imshow(window_title, frame)
            key = cv2.waitKey(1) & 0xFF
            if key in (ord("e"), ord("E")) and drone_controller is not None:
                drone_controller.emergency_land()
            if key in (ord("q"), 27):
                break
    finally:
...
def _load_drone_gesture_controller(
    *,
    required_stable_detections: int = DEFAULT_REQUIRED_STABLE_DETECTIONS,
    command_cooldown_detections: int = DEFAULT_COMMAND_COOLDOWN_DETECTIONS,
    lost_tracking_land_detections: int = DEFAULT_LOST_TRACKING_LAND_DETECTIONS,
